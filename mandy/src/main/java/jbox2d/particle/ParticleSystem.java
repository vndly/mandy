...
import jbox2d.dynamics.TimeStep;
...
public class ParticleSystem {
  ...
  jbox2d.dynamics.World m_world;

  public ParticleSystem(jbox2d.dynamics.World world) {
    m_world = world;
    m_timestamp = 0;
    ...
    if (m_count >= m_internalAllocatedCapacity) {
      ...
      if (m_internalAllocatedCapacity < capacity) {
        ...
        m_accumulationBuffer =
            jbox2d.common.BufferUtils.reallocateBuffer(m_accumulationBuffer, 0, m_internalAllocatedCapacity, capacity, false);
        m_accumulation2Buffer =
            jbox2d.common.BufferUtils.reallocateBuffer(Vec2.class, m_accumulation2Buffer, 0, m_internalAllocatedCapacity, capacity, true);
        m_depthBuffer =
            jbox2d.common.BufferUtils.reallocateBuffer(m_depthBuffer, 0, m_internalAllocatedCapacity, capacity, true);
        m_colorBuffer.data =
            reallocateBuffer(m_colorBuffer, m_internalAllocatedCapacity, capacity, true);
        m_groupBuffer =
            jbox2d.common.BufferUtils.reallocateBuffer(ParticleGroup.class, m_groupBuffer, 0, m_internalAllocatedCapacity, capacity, false);
...
//    assertNotSamePosition();
    ...
    if (m_proxyCount >= m_proxyCapacity) {
      ...
      m_proxyBuffer =
          jbox2d.common.BufferUtils.reallocateBuffer(Proxy.class, m_proxyBuffer, oldCapacity, newCapacity);
  ...
  private final jbox2d.collision.AABB temp = new jbox2d.collision.AABB();
  ...
  private final jbox2d.collision.AABB temp2 = new jbox2d.collision.AABB();
  ...
  public ParticleGroup createParticleGroup(ParticleGroupDef groupDef) {
    ...
    if (groupDef.shape != null) {
      ...
      jbox2d.collision.AABB aabb = temp;
      int childCount = shape.getChildCount();
      for (int childIndex = 0; childIndex < childCount; childIndex++) {
        if (childIndex == 0) {
          shape.computeAABB(aabb, identity, childIndex);
        } else {
          jbox2d.collision.AABB childAABB = temp2;
      ...
      for (float y = jbox2d.common.MathUtils.floor(aabb.lowerBound.y / stride) * stride; y < upperBoundY; y +=
          stride) {
        for (float x = jbox2d.common.MathUtils.floor(aabb.lowerBound.x / stride) * stride; x < upperBoundX; x +=
            stride) {
          Vec2 p = tempVec;
          p.x = x;
          p.y = y;
      ...
      for (int k = 0; k < m_contactCount; k++) {
        ...
        if (firstIndex <= a && b < lastIndex) {
          if (m_pairCount >= m_pairCapacity) {
            ...
            m_pairBuffer =
                jbox2d.common.BufferUtils.reallocateBuffer(Pair.class, m_pairBuffer, oldCapacity, newCapacity);
          ...
          pair.distance = jbox2d.common.MathUtils.distance(m_positionBuffer.data[a], m_positionBuffer.data[b]);
          m_pairCount++;
        }
      }
    }
    ...
    if ((particleFlags & k_pairFlags) != 0) {
      for (int k = 0; k < m_contactCount; k++) {
        ...
        if (groupA.m_firstIndex <= a && a < groupA.m_lastIndex && groupB.m_firstIndex <= b
          ...
          pair.indexB = b;
          pair.flags = contact.flags;
          pair.strength = jbox2d.common.MathUtils.min(groupA.m_strength, groupB.m_strength);
          pair.distance = jbox2d.common.MathUtils.distance(m_positionBuffer.data[a], m_positionBuffer.data[b]);
          m_pairCount++;
        }
      }
...
//    assert(d2 != 0);
    if (d2 < m_squaredDiameter) {
      if (m_contactCount >= m_contactCapacity) {
        ...
        m_contactBuffer =
            jbox2d.common.BufferUtils.reallocateBuffer(ParticleContact.class, m_contactBuffer, oldCapacity, newCapacity);
        m_contactCapacity = newCapacity;
      }
      float invD = d2 != 0 ? jbox2d.common.MathUtils.sqrt(1 / d2) : Float.MAX_VALUE;
  ...
  public void updateBodyContacts() {
    final jbox2d.collision.AABB aabb = temp;
  ...
  public void solve(TimeStep step) {
    ...
    for (int i = 0; i < m_count; i++) {
      ...
      if (v2 > criticalVelocytySquared) {
        float a = v2 == 0 ? Float.MAX_VALUE : jbox2d.common.MathUtils.sqrt(criticalVelocytySquared / v2);
        v.x *= a;
        v.y *= a;
      }
    }
  ...
  void solvePressure(TimeStep step) {
    ...
    for (int i = 0; i < m_count; i++) {
      ...
      float h =
          pressurePerWeight
              * jbox2d.common.MathUtils.max(0.0f, jbox2d.common.MathUtils.min(w, Settings.maxParticleWeight) - Settings.minParticleWeight);
    ...
    for (int k = 0; k < m_bodyContactCount; k++) {
      ParticleBodyContact contact = m_bodyContactBuffer[k];
      int a = contact.index;
      jbox2d.dynamics.Body b = contact.body;
      float w = contact.weight;
  ...
  private final jbox2d.common.Rot tempRot = new jbox2d.common.Rot();
  ...
  void solveRigid(final TimeStep step) {
    for (ParticleGroup group = m_groupList; group != null; group = group.getNext()) {
      if ((group.m_groupFlags & ParticleGroupType.b2_rigidParticleGroup) != 0) {
        ...
        Vec2 temp = tempVec;
        Vec2 cross = tempVec2;
        jbox2d.common.Rot rotation = tempRot;
        rotation.set(step.dt * group.m_angularVelocity);
        jbox2d.common.Rot.mulToOutUnsafe(rotation, group.m_center, cross);
  ...
  void solveElastic(final TimeStep step) {
    ...
    for (int k = 0; k < m_triadCount; k++) {
      ...
      if ((triad.flags & ParticleType.b2_elasticParticle) != 0) {
        ...
        float invR = r2 == 0 ? Float.MAX_VALUE : jbox2d.common.MathUtils.sqrt(1f / r2);
        rs *= invR;
        rc *= invR;
  ...
  void solveSpring(final TimeStep step) {
    ...
    for (int k = 0; k < m_pairCount; k++) {
      ...
      if ((pair.flags & ParticleType.b2_springParticle) != 0) {
        ...
        float r1 = jbox2d.common.MathUtils.sqrt(dx * dx + dy * dy);
  ...
  void solveZombie() {
    ...
    for (int i = 0; i < m_count; i++) {
      ...
      if ((flags & ParticleType.b2_zombieParticle) != 0) {
        jbox2d.callbacks.ParticleDestructionListener destructionListener = m_world.getParticleDestructionListener();
    ...
    for (ParticleGroup group = m_groupList; group != null; group = group.getNext()) {
      ...
      for (int i = group.m_firstIndex; i < group.m_lastIndex; i++) {
        j = newIndices[i];
        if (j >= 0) {
          firstIndex = jbox2d.common.MathUtils.min(firstIndex, j);
          lastIndex = jbox2d.common.MathUtils.max(lastIndex, j + 1);
        } else {
          modified = true;
        }
      }
  ...
  void RotateBuffer(int start, int mid, int end) {
    ...
    jbox2d.common.BufferUtils.rotate(m_flagsBuffer.data, start, mid, end);
    jbox2d.common.BufferUtils.rotate(m_positionBuffer.data, start, mid, end);
    jbox2d.common.BufferUtils.rotate(m_velocityBuffer.data, start, mid, end);
    jbox2d.common.BufferUtils.rotate(m_groupBuffer, start, mid, end);
    if (m_depthBuffer != null) {
      jbox2d.common.BufferUtils.rotate(m_depthBuffer, start, mid, end);
    }
    if (m_colorBuffer.data != null) {
      jbox2d.common.BufferUtils.rotate(m_colorBuffer.data, start, mid, end);
    }
    if (m_userDataBuffer.data != null) {
      jbox2d.common.BufferUtils.rotate(m_userDataBuffer.data, start, mid, end);
    }
  ...
  public void queryAABB(ParticleQueryCallback callback, final jbox2d.collision.AABB aabb) {
    if (m_proxyCount == 0) {
      return;
    }
   ...
   * @param point2
   */
  public void raycast(jbox2d.callbacks.ParticleRaycastCallback callback, final Vec2 point1, final Vec2 point2) {
    if (m_proxyCount == 0) {
      return;
    }
    int firstProxy =
        lowerBound(
            m_proxyBuffer,
            m_proxyCount,
            computeTag(m_inverseDiameter * jbox2d.common.MathUtils.min(point1.x, point2.x) - 1, m_inverseDiameter
                * jbox2d.common.MathUtils.min(point1.y, point2.y) - 1));
    int lastProxy =
        upperBound(
            m_proxyBuffer,
            m_proxyCount,
            computeTag(m_inverseDiameter * jbox2d.common.MathUtils.max(point1.x, point2.x) + 1, m_inverseDiameter
                * jbox2d.common.MathUtils.max(point1.y, point2.y) + 1));
    float fraction = 1;
    ...
    for (int proxy = firstProxy; proxy < lastProxy; ++proxy) {
      ...
      if (determinant >= 0) {
        float sqrtDeterminant = jbox2d.common.MathUtils.sqrt(determinant);
        ...
        fraction = jbox2d.common.MathUtils.min(fraction, f);
        if (fraction <= 0) {
          break;
        }
      }
  ...
  static <T> T[] reallocateBuffer(ParticleBuffer<T> buffer, int oldCapacity, int newCapacity,
    ...
    return jbox2d.common.BufferUtils.reallocateBuffer(buffer.dataClass, buffer.data, buffer.userSuppliedCapacity, oldCapacity, newCapacity, deferred);
  }

  static int[] reallocateBuffer(ParticleBufferInt buffer, int oldCapacity, int newCapacity,
      boolean deferred) {
    assert (newCapacity > oldCapacity);
    return jbox2d.common.BufferUtils.reallocateBuffer(buffer.data, buffer.userSuppliedCapacity, oldCapacity, newCapacity, deferred);
  }
      ...
      if (dabx * dabx + daby * daby < maxDistanceSquared
        ...
        if (system.m_triadCount >= system.m_triadCapacity) {
          ...
          system.m_triadBuffer =
              jbox2d.common.BufferUtils.reallocateBuffer(Triad.class, system.m_triadBuffer, oldCapacity, newCapacity);
      ...
      if (countA > 0 && countA < 3) {
        ...
        if ((af & bf & cf & k_triadFlags) != 0) {
          ...
          if (dabx * dabx + daby * daby < maxDistanceSquared
            ...
            triad.strength = jbox2d.common.MathUtils.min(groupA.m_strength, groupB.m_strength);
            final float midPointx = (float) 1 / 3 * (pa.x + pb.x + pc.x);
            final float midPointy = (float) 1 / 3 * (pa.y + pb.y + pc.y);
            triad.pa.x = pa.x - midPointx;
            triad.pa.y = pa.y - midPointy;
  ...
  static class UpdateBodyContactsCallback implements QueryCallback {
    ...
    public boolean reportFixture(Fixture fixture) {
      ...
      int childCount = shape.getChildCount();
      for (int childIndex = 0; childIndex < childCount; childIndex++) {
        jbox2d.collision.AABB aabb = fixture.getAABB(childIndex);
            ...
            if (d < system.m_particleDiameter) {
              ...
              if (system.m_bodyContactCount >= system.m_bodyContactCapacity) {
                ...
                system.m_bodyContactBuffer =
                    jbox2d.common.BufferUtils.reallocateBuffer(ParticleBodyContact.class, system.m_bodyContactBuffer, oldCapacity, newCapacity);
  ...
  static class SolveCollisionCallback implements QueryCallback {
    ...
    private final jbox2d.collision.RayCastOutput output = new jbox2d.collision.RayCastOutput();
    private final Vec2 tempVec = new Vec2();
    private final Vec2 tempVec2 = new Vec2();
    ...
    public boolean reportFixture(Fixture fixture) {
      ...
      final Shape shape = fixture.getShape();
      jbox2d.dynamics.Body body = fixture.getBody();
      int childCount = shape.getChildCount();
      for (int childIndex = 0; childIndex < childCount; childIndex++) {
        jbox2d.collision.AABB aabb = fixture.getAABB(childIndex);
        final float aabblowerBoundx = aabb.lowerBound.x - system.m_particleDiameter;
